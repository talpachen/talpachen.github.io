---
title: DDR 初始化
date: 2016-04-27 14:35:30
tags: [DDR]
---

##### 本文为DDR学习笔记，控制器IP为UMCTL，存储器是单片16bit ddr2，具体型号为H5PS2562GFR

# DDR芯片内部
### Bank及Row、Column描述
![](ddr_init/addr_table.PNG)

根据上表，可以计算出访问地址宽度：2^13 * 2^9 = 4M
共有4个bank，位宽为16，所以芯片容量：4M * 4 * 16 = 16M * 16

### 内部结构（截自另一块32M * 16芯片的数据手册）
![](ddr_init/block.PNG)

注意图中的行地址是A0-A9，这与正在讨论的芯片不同

# 地址映射关系
物理地址的映射关系是可以调整的，不过在UMCTL控制器上，没有发现相关配置寄存器，应该是默认使用了RBC模式。
```
------DDR------------------物理地址-----------------         
row       A12-A0    ->    ADDR[23:11]
bank      BA1-BA0   ->    ADDR[10:9]
column    A8-A0     ->    ADDR[8:0]
```

# 初始化逻辑
### 某DDR2初始化开源代码分析 [完整代码](https://chromium.googlesource.com/chromiumos/third_party/coreboot/+/chromeos-2013.04/src/soc/imgtec/pistachio/ddr2_init.c)
```c
...
int init_ddr2(void)
{
	...
	// 4B表示 rank[0] row[1] bank[1] column[3]
	// 对应参数：rank 1 row 14bit bank 8 column 10bit
	...
	// 设置 write recovery 及 cas latency
